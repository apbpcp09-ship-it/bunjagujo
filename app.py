    ...
    if st.button("🚀 훈련된 AI 기반 분자 생성", use_container_width=True):
        ...
        with torch.no_grad():
            ...
            for step in range(5):
                ...
                # 💡 핵심 수정: Temperature 기반 확률 분포 변환 후 무작위 주사위 샘플링 진행
                scaled_logits = logits / temperature
                probs = F.softmax(scaled_logits, dim=-1)
                
                # 무작위성을 유지하면서 과학적 필터를 통과시키기 위해 다중 샘플링 후보 추출
                # 10번 주사위를 굴려 확률이 높은 후보들을 랜덤하게 나열
                sampled_indices = torch.multinomial(probs, num_samples=10, replacement=True)
                ...
                # 주사위 결과 후보군 중에서 과학적 규칙을 최초로 만족하는 글자를 선택
                for idx in sampled_indices:
                    char = idx_to_char[idx.item()]
                    if char == '<pad>': continue
                    if char == '<eos>':
